...
| 方法 | 路径 | 说明 | 鉴权 |
| --- | --- | --- | --- |
| GET | `/summary` | 月度收支汇总 | 是 |
| GET | `/category-pie` | 分类支出饼图数据 | 是 |
| GET | `/timeline` | 消费趋势时间轴 | 是 |

**GET /api/stats/summary** Query: `month=2026-06`
响应：
```json
{ "code":0, "data": { "income_total":5000, "expense_total":3200, "balance":1800 } }
```
...
│  渲染层:                                   │
│  1. 背景渐变 (每帧重绘)                    │
│  2. 小球 (z-index: 低, 静态+已配置标记)    │
│  3. 糖豆人 (z-index: 中, 逐帧移动+动画)    │
│  4. 星星粒子 (z-index: 高, 进食特效)       │
│  5. 余额 HUD (z-index: 顶层, 固定位置)    │
│                                           │
...
点击糖豆人 → 触发 panel.open():
  ...
  2. 从右侧 transform 滑入面板 DOM
  3. 面板内发起 API 请求加载今日账单
...
| JWT 签名 | HS256，服务端 `JWT_SECRET` 环境变量，默认值仅开发用 |
...
| T3-6 | 账单编辑/删除：长按触发编辑、左滑删除按钮 | 可编辑删除 |
...
> **变更说明**：v1.1 - 确认所有技术决策项，补充关键设计决策章节
